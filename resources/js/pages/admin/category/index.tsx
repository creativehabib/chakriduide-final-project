...
import { CategoryType, FlashProps, LinksType, MetaType } from '@/types/globals';
...
export default function CategoriesIndex(){
    ...
    const { flash, categories } = usePage<{ flash: FlashProps; categories: categoriesType }>().props;
    const [openDeleteDialog, setOpenDeleteDialog] = useState(false);
    const [blogToDelete, setBlogToDelete] = useState<number | null>(null);

    const handleDelete = () => {
        if (blogToDelete !== null) {
            router.delete(route('blog.categories.destroy', blogToDelete), {
                preserveScroll: true,
                preserveState: true,
                onSuccess: () => {
            ...
            <AppLayout breadcrumbs={breadcrumbs}>
                ...
                <div className="p-6">
                    ...
                    <div>
                        <Table className="border">
                            <TableHeader>
                                <TableRow>
                                    ...
                                    <TableHead>Name with post count</TableHead>
                                ...
                                {loading ? (
                                    ...
                                    categories.data.map((post, index) => (
                                        <TableRow key={post.id}>
                                            ...
                                            <TableCell>
                                                <Permalink slug={post.slug} name={post.name}/>({post.blogs.length})
                                            </TableCell>
                        ...
                        <div className="mb-2">
                            <InertiaPagination meta={categories.meta} />
                        </div>
                    </div>
                </div>
            </AppLayout>
